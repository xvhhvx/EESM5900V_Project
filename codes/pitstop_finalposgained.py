...
## Trick to run by GPU, rather than CPU ##
import os
...
def excel_unboxer(file_location):
    ...
    # reorder the columns
    combined_table = combined_table[["No.", "initial_pos", "no_of_pits", "tyre_grid", "tyre_1", "tyre_2", "tyre_grid_distance", "tyre_1_distance", "tyre_2_distance", "sc", "Temperature", "Humidity", "Altitude", "Turns", "RaceDistance", "TrackLength", "TeamAbility", "DriverAbility", "final_pos_gained"]]
    ...
    combined_table = combined_table[["initial_pos", "no_of_pits", "tyre_grid", "tyre_1", "tyre_2", "tyre_grid_distance", "tyre_1_distance", "tyre_2_distance", "sc", "Temperature", "Humidity", "Altitude", "Turns", "RaceDistance", "TrackLength", "TeamAbility", "DriverAbility", "final_pos_gained"]]
...
drop_indices = np.random.choice(to_be_deleted_index_0, 99, replace=False)
dataset = dataset.drop(drop_indices)
drop_indices = np.random.choice(to_be_deleted_index_n1, 19, replace=False)
dataset = dataset.drop(drop_indices)
drop_indices = np.random.choice(to_be_deleted_index_1, 55, replace=False)
dataset = dataset.drop(drop_indices)
drop_indices = np.random.choice(to_be_deleted_index_2, 27, replace=False)
dataset = dataset.drop(drop_indices)

dataset.to_csv('dataset_pitstop.csv')
...
def custom_accuracy(y_true, y_pred):
    return keras.backend.mean(keras.backend.less_equal(keras.backend.abs(y_true - y_pred), 2))

noOfEpochs = 1000

model.compile(loss='mean_absolute_error', optimizer='rmsprop', metrics=[custom_accuracy])
...
plt.xlabel('Epoch Number')
plt.ylabel("Accuracy")
plt.plot(range(noOfEpochs), history.history['custom_accuracy'], label='accuracy')
...
results = model.evaluate(test_features, test_labels, batch_size=128, verbose=1) #128-64
